...
try {
    ...
    // 상품 기본 정보 조회
    String sql = "SELECT c.prdName, c.prdImage, pd.prdDesign " +
...
%>
    <div class="main">
            <h2>후기 작성하기</h2>
			<form method="post" action="InsertReview_Ok.jsp" enctype="multipart/form-data" class="review-form">

        <input type="hidden" name="prdNo" value="<%= prdNoParam %>" />
		<input type="hidden" name="ordNo" value="<%= request.getParameter("ordNo") %>" />

		<%
    String imagePath = "images/" + prdImage;
    if (prdNo >= 61) {
        imagePath = "images/designCakeImage.jpg"; // 기본 디자인 케이크 이미지

        if ("도안 케이크".equals(prdName) && prdDesign != null && !prdDesign.isEmpty()) {
            imagePath = "/onlycake/uploads/" + prdDesign; // 도안 케이크일 경우 업로드된 이미지 사용
        }
    }
%>
...
<div class="star-rating" id="starRating">
    <img src="images/star_brown.png" class="star" data-value="1" />
    <img src="images/star_brown.png" class="star" data-value="2" />
    <img src="images/star_brown.png" class="star" data-value="3" />
    <img src="images/star_brown.png" class="star" data-value="4" />
    <img src="images/star_brown.png" class="star" data-value="5" />
</div>
<input type="hidden" name="rating" id="ratingValue" value="0" />

        <textarea name="reviewContent" placeholder="후기를 작성해주세요..."></textarea>

        <div class="photo-upload">
            <label for="photoUpload">사진 첨부하기</label>
	...
	<label>비밀번호</label>
		<div class="pass">
			<input type="password" name="reviewPwd" placeholder="비밀번호를 등록해주세요" id="Qpass" required>
		</div>
	</div>
    ...
    stars.forEach(star => {
        star.addEventListener("click", function () {
            ...
            stars.forEach(s => {
                ...
                if (v <= selectedRating) {
                    s.src = "images/star_yellow.png";
                } else {
                    s.src = "images/star_brown.png";
                }
            });
        });
    });
