...
nav {
    ...
    align-items: center;
    margin-left: -27px; /* 메뉴를 왼쪽으로 조금 이동 */
...
window.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("naverPayBtn").addEventListener("click", function () {
        ...
        // 배송/결제 정보 가져오기
        const recipient = document.getElementById("ordrecipient").value;
        const addr1 = document.getElementById("ordRcvaddress1").value;
        const addr2 = document.getElementById("ordRcvaddress2").value;
        const phone = document.getElementById("ordRcvphone").value;
        const requestText = document.getElementById("shippingRequest").value;
        const bank = document.getElementById("ordBank").value;
        const bankname = document.getElementById("ordBankname").value;

        fetch("StoreNaverSession.jsp", {
            method: "POST",
            body: formData
        }).then(() => {
            const returnUrl = "http://localhost:8080/onlycake/PayNaverCallback.jsp?" +
                "ordNo=<%= ordNo %>" +
                "&ctNos=<%= ctNosParam %>" +
                "&ordPay=<%= finalAmount %>" +
                "&totalQuantity=<%= totalQuantity %>" +
                "&ordrecipient=" + encodeURIComponent(recipient) +
                "&ordRcvaddress1=" + encodeURIComponent(addr1) +
                "&ordRcvaddress2=" + encodeURIComponent(addr2) +
                "&ordRcvphone=" + encodeURIComponent(phone) +
                "&shippingRequest=" + encodeURIComponent(requestText) +
                "&ordBank=" + encodeURIComponent(bank) +
                "&ordBankname=" + encodeURIComponent(bankname);
...
<form action="PayOK.jsp" method="post" class="shippingform">
    ...
    <div class="pay_button_container">
        ...
        <button class="pay_select2" type="button" id="naverPayBtn">네이버페이</button>
    ...
    <button type="submit" class="pay_button">결제완료</button>
</form>
</div>
