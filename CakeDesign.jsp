...
<div class="wrapper">   <!--3D 공간과 옵션폼 감싸는 공간-->
    <canvas id="canvas"></canvas>   <!--3D 공간-->
    
    <div class="content1">    
        <div class="cake_detail">
            <p style="color:#C31D3B; font-size: 25px; font-family: 'NanumSquareNeoExtraBold', sans-serif;">디자인 케이크</p>
            <p style="color:#744731; font-size: 20px; font-family: 'NanumSquareNeoBold', sans-serif;">22,000원</p>
            <p style="color:#744731; font-size: 16px;">디자인케이크의 대략적인 디자인은 이미 결정되어
            있으며, 케이크의 윗면에 레터링(문구) 
            커스터마이징이 가능합니다. 지정 날짜로부터 약 1~2일 뒤에 택배 수령 가능합니다.</p>
    ...
    <div class="form_container">   <!--옵션폼-->
		...
		<input type="hidden" name="prdName" value="디자인 케이크">
		<input type="hidden" name="prdPrice" value="22000">
		<input type="hidden" name="prdImage" value="designCakeImage.jpg">
            <label for="cake_size" style="margin-top: 20px;">케이크 사이즈</label>
            <select id="cake_size" name="prdCakesize" onchange="updateSizePrice()">
                <option value="1호(+0)" data-price="0">1호(+0)</option>
                <option value="2호(+5,000)" data-price="5000">2호(+5,000)</option>
                <option value="3호(+10,000)" data-price="10000">3호(+10,000)</option>
            </select>
            ...
            <label for="cake_fruit">장식 과일</label>
            <select id="cake_fruit" name="prdFruit" onchange="updateFruitPrice()">
                <option value="과일 추가 안함" data-price="0">과일 추가 안함(+0)</option>
                <option value="딸기(+5,000)" data-price="5000">딸기(+5,000)</option>
                <option value="체리(+5,000)" data-price="5000">체리(+5,000)</option>
                <option value="망고(+5,000)" data-price="5000">망고(+5,000)</option>
            </select>
			<input type="hidden" name="prdFruitprice" id="prdFruitprice" value="0">
            
            <label for="cake_topcream">상단 데코레이션</label>
            <select id="cake_topcream" name="prdTopcream">
                <option value="별깍지 모양">별깍지 모양</option>
                <option value="원형 모양">원형 모양</option>
                <option value="회오리 모양">회오리 모양</option>
            </select>
            
            <label for="cake_bottomcream">하단 데코레이션</label>
            <select id="cake_bottomcream" name="prdBottomcream">
                <option value="별깍지 모양">별깍지 모양</option>
                <option value="원형 모양">원형 모양</option>
                <option value="회오리 모양">회오리 모양</option>
            </select>
...
<select id="cake_candle" name="prdCandle" onchange="updateCandlePrice()"> 
    <option value="초 구매 안함" data-price="0">초 구매 안함(+0)</option>
...
<input type="hidden" name="prdCoolingprice" id="prdCoolingprice" value="0">
    ...
    const candleSelect = document.getElementById("cake_candle");
...
function updateCoolingPrice() {
    const coolingSelect = document.getElementById("cake_cooling");
...
function updateSizePrice() {
    ...
    document.getElementById("prdSizeprice").value = selectedOption.getAttribute("data-price");
}

function updateFruitPrice() {
    const fruitSelect = document.getElementById("cake_fruit");
    const selectedOption = fruitSelect.options[fruitSelect.selectedIndex];
    document.getElementById("prdFruitprice").value = selectedOption.getAttribute("data-price");
}
...
function formatPrice(price) {
    ...
    <script>
        ...
        camera.position.set(1, 1.5, 1.0);

        const renderer = new THREE.WebGLRenderer({ canvas: document.getElementById('canvas') });
        renderer.setSize(600, 600);
        renderer.setClearColor(0xFFF6ED); // 배경 색상 설정
        ...
        let currentFruitModel = null; // 현재 로드된 과일 모델
        let currentTopCreamModel = null; // 현재 로드된 위쪽 크림 모델
        let currentBottomCreamModel = null; // 현재 로드된 아래 크림 모델
        let currentCakeBaseModel = null; // 현재 로드된 케이크 기본 모델

        // 모델 로드 함수
        function loadModel(modelPath, modelType) {
            // 기존 모델이 있으면 씬에서 제거
            if (modelType === 'fruit' && currentFruitModel) {
                scene.remove(currentFruitModel);
            } else if (modelType === 'topCream' && currentTopCreamModel) {
                scene.remove(currentTopCreamModel);
            } else if (modelType === 'bottomCream' && currentBottomCreamModel) {
                scene.remove(currentBottomCreamModel);
            } else if (modelType === 'cakeBase' && currentCakeBaseModel) {
                scene.remove(currentCakeBaseModel);
            }
            ...
            loader.load(
                ...
                function (gltf) {
                    if (modelType === 'fruit') {
                        currentFruitModel = gltf.scene;
                        scene.add(currentFruitModel);
                    } else if (modelType === 'topCream') {
                        currentTopCreamModel = gltf.scene;
                        scene.add(currentTopCreamModel);
                    } else if (modelType === 'bottomCream') {
                        currentBottomCreamModel = gltf.scene;
                        scene.add(currentBottomCreamModel);
                    } else if (modelType === 'cakeBase') {
                        currentCakeBaseModel = gltf.scene;
                        scene.add(currentCakeBaseModel);
                    }
                },
                undefined,
                function (error) {
        ...
        // 드롭다운 선택 이벤트 리스너 (과일)
        // 과일 선택 이벤트
document.getElementById("cake_fruit").addEventListener("change", function() {
    const selectedValue = this.value;

    let modelFile = '';
    switch (selectedValue) {
        case '딸기(+5,000)':
            modelFile = '/onlycake/object/Strawberry.gltf';
            break;
        case '체리(+5,000)':
            modelFile = '/onlycake/object/Cherry.gltf';
            break;
        case '망고(+5,000)':
            modelFile = '/onlycake/object/Mango.gltf';
            break;
        case '과일 추가 안함':
            if (currentFruitModel) {
                scene.remove(currentFruitModel);
                currentFruitModel = null;
            }
            return;
        default:
            return;
    ...
    loadModel(modelFile, 'fruit');
});

// 상단 크림 선택 이벤트
document.getElementById("cake_topcream").addEventListener("change", function() {
    const selectedValue = this.value;

    let modelFile = '';
    switch (selectedValue) {
        case '별깍지 모양':
            modelFile = '/onlycake/object/Topcream1.gltf';
            break;
        case '원형 모양':
            modelFile = '/onlycake/object/Topcream2.gltf';
            break;
        case '회오리 모양':
            modelFile = '/onlycake/object/Topcream3.gltf';
            break;
        default:
            return;
    }

    loadModel(modelFile, 'topCream');
});

// 하단 크림 선택 이벤트
document.getElementById("cake_bottomcream").addEventListener("change", function() {
    const selectedValue = this.value;

    let modelFile = '';
    switch (selectedValue) {
        case '별깍지 모양':
            modelFile = '/onlycake/object/Bottomcream1.gltf';
            break;
        case '원형 모양':
            modelFile = '/onlycake/object/Bottomcream2.gltf';
            break;
        case '회오리 모양':
            modelFile = '/onlycake/object/Bottomcream3.gltf';
            break;
        default:
            return;
    }

    loadModel(modelFile, 'bottomCream');
});
...
document.getElementById("cake_flavor").addEventListener("change", function() {
    ...
    switch (selectedValue) {
        case '바닐라시트 + 우유 생크림':
        case '초코시트 + 우유 생크림':
            modelFile = '/onlycake/object/Cakebreadwhite.gltf';
            break;
        case '바닐라시트 + 초코 생크림':
        case '초코시트 + 초코 생크림':
            modelFile = '/onlycake/object/breadbrown.gltf';
            break;
        case '바닐라시트 + 녹차 생크림':
        case '초코시트 + 녹차 생크림':
            modelFile = '/onlycake/object/breadgreen.gltf';
            break;
        default:
            return;
    }

    loadModel(modelFile, 'cakeBase');
});




        // 초기 로드: Cakebreadpad, Topcream1, Bottomcream1 모델 로드
        loadModel('/onlycake/object/Cakebreadwhite.gltf', 'cakeBase');
        loadModel('/onlycake/object/Topcream1.gltf', 'topCream');
        loadModel('/onlycake/object/Bottomcream1.gltf', 'bottomCream');
